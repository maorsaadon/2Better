...
const NotificationCard = ({ notification }) => {
  ...
  const request =
    type === "Group Join request" || type === "Meeting Join request"
      ? true
      : false;
  const [handled, setHandled] =
    type === "Group Join request"
      ? useState(notification.Handled)
      : useState(false);
  const [requestAnswer, setRequestAnswer] =
    type === "Group Join request"
      ? useState(notification.RequestAnswer)
      : useState("");
    ...
    <SafeAreaView>
      <View style={NotificationCardStyles.card}>
        ...
        {request && !handled ? (
          ...
          <View style={NotificationCardStyles.cardBottomRow}>
            <Text style={NotificationCardStyles.acceptText}>
              You've accepted the request
            </Text>
          </View>
        ) : request && requestAnswer == "Rejected" ? (
          <View style={NotificationCardStyles.cardBottomRow}>
            <Text style={NotificationCardStyles.rejectText}>
